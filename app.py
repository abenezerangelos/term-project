...
def initDB(*args, **kwargs):
    with app.app_context():
        ...
        if Researchfield.query.count() == 0:
            rfields = ['Artifical Intelligence', 'Machine Learning', 'Software Engineering', 'High Performance Computing', 'Distributed and Networked Systems']
            for rf in rfields:
        ...
        if Student.query.count() == 0:
            ...
            student1.programming_languages.append(Programminglanguage.query.filter_by(id=1).first())
            student1.research_fields.append(Researchfield.query.filter_by(id=4).first())
        ...
        if Application.query.count() == 0:  
            ...
            faculty_name="Faculty Name",
            faculty_email="faculty_email@example.com",
            studentid=student2.id,  # Link the application to the student1
            positionid=position2.id,  # Link the application to the position1
            status="Pending",  # Set the status 
            )
            # Add the application to the student's applications and positionsapplied lists
            student2.applications.append(application2)
            position1.submissions.append(application2)
